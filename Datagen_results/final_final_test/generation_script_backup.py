...
for index, sample in enumerate(original_scale_samples):
    ...
    cmd_pars = [{'par': 'l1.L', 'val': str(L_pero)}, {'par':'l1.bulkTrapFile', 'val':str(multitrap_filename)},
                ...
                {'par':'useExpData','val':str(1)}, {'par':'expJV','val':'exp_JV.csv'},
...
df_params = pd.DataFrame(original_scale_samples, columns=columns[1:])
df_params.insert(0, 'ID', ID_list)
df_params.to_csv(os.path.join(res_dir, 'parameters_ID_list.csv'), index=False)
del df_params, columns
