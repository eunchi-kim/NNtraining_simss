...
varied_parameters = {                                                       
    'W_L': (4.21, 4.5),  # Left defect position range                       
    'W_R': (4.21, 4.5),  # Right defect position range                      
    'E_d,sh': (4.22, 4.48),  # Defect energy range                          
    'N_t,sh': (1e22, 1e24),  # Defect density range                         
    'N_t,d': (1e20, 1e22),  # Defect density range                          
    'mu_e': (1e-8, 1e-4)    # electron mobility                             
}                                                                           
...
sobol = Sobol(d=len(varied_parameters), scramble=True)
...
cmd_pars_list, ID_list, JV_data_list = [], [], []
for index, sample in enumerate(original_scale_samples):
    ...
    cmd_pars = [{'par': 'l1.L', 'val': str(L_pero)}, {'par':'l1.bulkTrapFile', 'val':str(multitrap_filename)},
                {'par': 'l1.mu_n', 'val': str(mu_e)},
                {'par':'Vmax','val':str(Vmax)},{'par':'NP','val':str(NP)},{'par':'Vscan','val':str(Vscan)},
                {'par':'tolPois','val':str(tolPois)},{'par':'tolDens','val':str(tolDens)},{'par':'couplePC','val':str(couplePC)},
                {'par':'minAcc','val':str(minAcc)},{'par':'maxAcc','val':str(maxAcc)},{'par':'grad','val':str(grad)}]
    cmd_pars_list.append(cmd_pars)
    ID_list.append(str(uuid.uuid4()))

# Run the simulations in parallel
Parallel(n_jobs=min(len(cmd_pars_list),15))(delayed(run)(ID, cmd_pars, simss_device_parameters, res_dir) for ID, cmd_pars in zip(ID_list, cmd_pars_list))    
...
h5py_filename = os.path.join(res_dir, 'SCLC_Dataset.h5')
