...
import simss_utils.general as utils_gen
from simss_utils.parallel_sim import *
from simss_utils.utils import update_cmd_pars

######### Functions #################################################################################
def run_SS_JV(simss_device_parameters, result_path, session_path, JV_file_name = 'JV.dat', G_fracs = [], parallel = False, max_jobs = max(1,os.cpu_count()-1), run_mode = True, **kwargs):
    """
    ...
    **kwargs : dict
    ...
    if G_fracs is None:
        ...
        JV_file_name = os.path.join(result_path,JV_file_name)
        logFile = os.path.join(result_path,'log.txt')
        # scParsFile = os.path.join(result_path,'scPars.txt')
        if UUID != '':
            JV_file_name_base, JV_file_name_ext = os.path.splitext(JV_file_name)
            # JV_file_name = JV_file_name_base + dum_str + JV_file_name_ext
            JV_file_name = os.path.join(result_path,'JV'+dum_str+'.dat')
            logFile = os.path.join(result_path,'log'+dum_str+'.txt')
            # scParsFile = os.path.join(result_path,'scPars'+dum_str+'.txt')
            # VarFile = os.path.join(result_path,'Var'+dum_str+'.txt')
        ...
        result, message = utils_gen.run_simulation('simss',SS_JV_args,session_path,run_mode = run_mode, verbose=False)
        # result, message = utils_gen.run_simulation_filesafe('simss', SS_JV_args, session_path, run_mode = False, verbose = False)
